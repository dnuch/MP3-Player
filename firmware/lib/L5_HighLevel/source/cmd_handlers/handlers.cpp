...
//        if (4 != cmdParams.scanf("%*s %0x %0x %0x %u", &addr, &reg, &data, &count)) {
        if (3 != cmdParams.scanf("%*s %0x %0x %0x", &addr, &reg, &data)) {
            output.putline("Need device, register address and data");
            return false;
        }

//        if (I2C2::getInstance().writeRegisters(addr, reg, data, count)) {
...
/// play mp3 from terminal task
QueueHandle_t sdFileCmdTaskHandle;
CMD_HANDLER_FUNC(playMP3Cmd)
{
    if (cmdParams.getLen() < 4) return false; /* no .txt/.mp3 extension */
