...
void nextOrPreviousFromISR(void) {
    long yield = 0;
    xSemaphoreGiveFromISR(xSemaphore[1][SW_P2_4], &yield);
    portYIELD_FROM_ISR(yield);
}
...
bool Init_MP3_GPIO_Interrupts() {
    ...
    gpioInterrupts.AttachInterruptHandler(portTwo, SW_P2_4, nextOrPreviousFromISR,      kFallingEdge);
    return true;
}
